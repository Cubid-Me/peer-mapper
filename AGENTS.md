...
- `tool-versions.md` — authoritative record of Node/pnpm/Foundry versions
- `.env.example`, `indexer/.env.example`, `frontend/.env.example` — env templates that must be kept in sync with new configuration
...
_Note: If any mandatory files under `.github/workflows/` or `agent-context/` are missing, agents should follow the bootstrap steps in `agent-context/agent-setup.md` (or the canonical upstream referenced there) to create them._
...
- **Env handling**
  - Update `.env.example` with _names_ of new vars (never values).
